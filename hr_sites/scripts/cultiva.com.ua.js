// source: https://cultiva.com.ua/
// extracted: 2026-05-07T21:18:56.940Z
// scripts: 2
...
document.addEventListener('click', function(e) {
  if (window.innerWidth <= 767) { // лише мобільна версія
    const nextPrev = e.target.closest('.j-catalog-pagination-btn'); // "Вперед"/"Назад"
    const loadMore = e.target.closest('.j-catalog-load-more-btn');  // "Показати ще"

    // Тільки якщо натиснуто "Вперед/Назад", без "Показати ще"
    if (nextPrev && !loadMore) {
      // невелика пауза перед скролом
      setTimeout(() => {
