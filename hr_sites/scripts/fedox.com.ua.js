// source: https://fedox.com.ua/
// extracted: 2026-05-07T21:19:16.742Z
// scripts: 2

// === script #1 (length=3044) ===
(function() {
  var R = '4.7', C = '64';
  var U = 'https://www.google.com/maps/reviews/@46.4323562,30.7591564,942m/data=!3m1!1e3!4m6!14m5!1m4!2m3!1sCi9DQUlRQUNvZENodHljRjlvT2pCbloyUTJZMjFrT1hwNloyMVhiMmxLUVd0Q2RWRRAB!2m1!1s0x0:0xd3a2d62853b12719?entry=ttu&g_ep=EgoyMDI2MDMyNC4wIKXMDSoASAFQAw%3D%3D'; // ЗАМЕНИ на свой Google Maps URL

  if (!document.querySelector('.product__section--order, .product__block--orderBox')) return;

  var s = document.createElement('style');
  s.textContent =
    '.fgr{margin:12px 0 16px;padding:14px 16px;border:1px solid #e0e0e0;border-radius:8px;background:#fff;display:flex;align-items:center;gap:12px;flex-wrap:wrap;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,sans-serif}' +
    '.fgr:hover{box-shadow:0 2px 8px rgba(0,0,0,.07)}' +
    '.fgr-r{font-size:20px;font-weight:700;color:#222}' +
    '.fgr-s{color:#FBBC04;font-size:17px;letter-spacing:1px}' +
    '.fgr-c{font-size:13px;color:#555}' +
    '.fgr-l{margin-left:auto;font-size:13px;color:#1a73e8;text-decoration:none;padding:6px 14px;border:1px solid #dadce0;border-radius:4px}' +
    '.fgr-l:hover{background:#f1f3f4;text-decoration:none}' +
    '@media(max-width:767px){.fgr{gap:8px}.fgr-l{margin-left:0}}';
  document.head.appendChild(s);

  var d = document.createElement('div');
  d.className = 'fgr';
  d.innerHTML =
    '<a href="'+U+'" target="_blank" rel="noopener" style="display:flex;align-items:center;text-decoration:none">' +
    ...
    '</a>' +
    '<span class="fgr-r">'+R+'</span>' +
    '<span class="fgr-s">\u2605\u2605\u2605\u2605\u2605</span>' +
    '<span class="fgr-c">'+C+' \u0432\u0456\u0434\u0433\u0443\u043a\u0456\u0432 \u043d\u0430 Google</span>' +
    '<a href="'+U+'" target="_blank" rel="noopener" class="fgr-l">\u0414\u0438\u0432\u0438\u0442\u0438\u0441\u044c \u0432\u0456\u0434\u0433\u0443\u043a\u0438</a>';

  // Десктоп: в левую колонку перед "Опис"
  var leftCol = document.querySelector('.j-product-left-column');
  if (leftCol && leftCol.children.length > 1) {
    leftCol.insertBefore(d, leftCol.children[1]);
    return;
  }

  // Мобильная: перед блоком "Опис" (product__block--description)
  var descBlock = document.querySelector('.product__block--description');
  if (descBlock) {
    descBlock.parentElement.insertBefore(d, descBlock);
    return;
  }
})();

// === script #2 (length=11492) ===
(function() {
  var PAY_API = 'https://pay.fedox.com.ua/api/pay';

  // === PART 2: /checkout/complete/ — auto-create Mono invoice ===
  if (window.location.pathname.indexOf('/checkout/complete/') > -1) {
    var saved = null;
    try { saved = JSON.parse(localStorage.getItem('fedox_pay_data')); } catch(e) {}
    if (saved && saved.ts && (Date.now() - saved.ts < 120000)) {
      localStorage.removeItem('fedox_pay_data');
  ...
  // Helper: check if element contains quick-order text (UA or RU)
  function isQuickOrderBlock(el) {
    var t = el.textContent;
    return t.indexOf('\u043a\u043b\u0456\u043a') > -1 ||       // "клік" (UA)
           t.indexOf('\u0411\u044b\u0441\u0442\u0440\u044b\u0439') > -1 || // "Быстрый" (RU)
           t.indexOf('\u0428\u0432\u0438\u0434\u043a') > -1;   // "Швидк" (UA alt)
  }

  var inserted = false;

  // DESKTOP: find quick-order block in product-order grid
  ...
  // MOBILE: find quick-order in product-card layout
  if (!inserted) {
  ...
  document.getElementById('fpm-go').addEventListener('click', function() {
    ...
    localStorage.setItem('fedox_pay_data', JSON.stringify({
