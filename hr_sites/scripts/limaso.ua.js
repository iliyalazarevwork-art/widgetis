// source: https://limaso.ua/
// extracted: 2026-05-07T21:19:26.270Z
// scripts: 1

// === script #1 (length=6090) ===
(function() {
    if (window.GLOBAL && GLOBAL.SYSTEM_LANGUAGE !== undefined &&
        GLOBAL.SYSTEM_LANGUAGE !== 3 && GLOBAL.SYSTEM_LANGUAGE !== 1) {
        return;
    }

    // Якщо бачимо РРЦ на поточній сторінці — запам'ятовуємо що клієнт оптовий/дроп
    if (document.querySelector('.catalogCard-rrpPrice, .catalog-card__rrp-price')) {
        sessionStorage.setItem('is_wholesale', '1');
        return;
    }

    // Якщо раніше бачили РРЦ — не показуємо прогрес бар
    if (sessionStorage.getItem('is_wholesale') === '1') return;

    var FREE_SHIPPING_THRESHOLD = 3500;
    ...
    function addFreeShippingProgressBar() {
        // Додаткова перевірка при кожному оновленні
        if (document.querySelector('.catalogCard-rrpPrice, .catalog-card__rrp-price')) {
            sessionStorage.setItem('is_wholesale', '1');
        ...
        progressBar.style.background = "#CB8D53";
            ...
            try {
                ...
                if (cart && cart.attachEventHandlers) {
                    cart.attachEventHandlers({
                        afterRender: function() { debounceAddProgressBar(500); },
                        cart_product_add: function() { debounceAddProgressBar(500); },
                        cart_product_remove: function() { debounceAddProgressBar(500); },
                        onChange: function() { debounceAddProgressBar(500); }
                    });
                }
            } catch (e) {
